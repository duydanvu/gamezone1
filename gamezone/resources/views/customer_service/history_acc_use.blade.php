        ...
        <div class="card-body">
            <table id="example1" class="table table-bordered table-striped " style="width: 100%">
                ...
                <tr>
                    ...
                    <th style="width:15%;text-align: center" >Thời gian thực hiện</th>
                ...
                <tbody id="table_body">
                @if ( count($history_acc_use) > 0)
                    @foreach($history_acc_use as $key => $value)
                        <tr>
                            <td>{{$key + 1}}</td>
                            <td>{{$value->isdn}}</td>
                            <td>{{$value->reg_datetime}}</td>
                            <td>{{$value->message_send}}</td>
                            <td>{{$value->request}}</td>
                            <td>{{$value->channel}}</td>
...
@section('js')
    ...
    <script>
        ...
        $('#export_data').click(function () {
            ...
            var url = "{{ route ('export_accuse_to_file_csv',['datetime' => ":datetime"])}}";
        ...
        $(document).ready(function(){
            $('#fillter_date').click(function () {
                ...
                $.ajax({
                    type:'POST',
                    url:'{{route('search_date_time_his_use')}}',
                    data:dt,
